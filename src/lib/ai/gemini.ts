...
3e. Every chart must include the source table/figure number when available, exact units, whether values are mean, mean ± SD, percentage, or score, and p-values only when present in the source.
3f. For visual acuity charts using logMAR, always state that lower logMAR values indicate better visual acuity. For discrete visual acuity endpoints such as UDVA, CDVA, UIVA, DCIVA, UNVA, and DCNVA, NEVER use a line chart. Use a grouped bar chart only (chartType "bar" with multiple datasets). Line charts are allowed only for true ordered curves such as defocus curves across diopters.
3g. In "content", place each figure placeholder exactly once, e.g. ${getFigurePlaceholderUrl(1)}.`
      ...
      try
      {
        const expansion = await generateContentWithFallback({
          ...
          prompt:
            ...
            `- Expand subgroup findings already mentioned, adding exact n, endpoint values, and p-values from source tables only when those values are explicitly present.\n` +
